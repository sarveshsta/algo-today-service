...
class MaxMinOfLastTwo(IndicatorInterface):
    ...
    SECOND_LAST_OHLC = 5
    CURRENT_OHLC = 5
    trade_details = {"done": False, "index": None}
    ...
    def check_indicators(self, data: pd.DataFrame, token:pd.DataFrame,  index: int = 0) -> tuple[Signal, float]:
        ...
        if not self.to_buy:
            if current_high >= (last_second_high + (last_second_high * 0.01)) or current_high >= (last_third_high + (last_third_high * 0.01)):
                print("CH/CH1/CH2", current_high, last_second_high, last_third_high)
                self.to_buy = True
        ...
        # condition to sell 
            ...
            if (data["High"].iloc[-1] >= self.price * 1.10) or \
                    (data["Low"].iloc[-1] <= self.price * 0.95):
...
instrument_reader = OpenApiInstrumentReader(NFO_DATA_URL, ["FINNIFTY09APR2421500PE", "BANKNIFTY10APR2448200CE"])
