...
class SmartWebSocketV2(object):
    ...
    def __init__(self, auth_token, api_key, client_code, feed_token, max_retry_attempt=10,retry_strategy=0, retry_delay=10, retry_multiplier=2, retry_duration=60):
        """
    ...
    def _parse_binary_data(self, binary_data):
        parsed_data = {
            ...
            "token": self._parse_token_value(binary_data[2:8]),
            "sequence_number": self._unpack_data(binary_data, 24, 28, byte_format="I")[0],
            "exchange_timestamp": self._unpack_data(binary_data, 28, 32, byte_format="I")[0],
            "last_traded_quantity": self._unpack_data(binary_data, 51, 59, byte_format="q")[0],
            "average_traded_price": self._unpack_data(binary_data, 59, 67, byte_format="q")[0],
            "volume_trade_for_the_day": self._unpack_data(binary_data, 67, 75, byte_format="q")[0],
            "total_buy_quantity": self._unpack_data(binary_data, 75, 83, byte_format="d")[0],
            "total_sell_quantity": self._unpack_data(binary_data, 83, 91, byte_format="d")[0],
            "open_price_of_the_day": self._unpack_data(binary_data, 91, 99, byte_format="q")[0],
            "high_price_of_the_day": self._unpack_data(binary_data, 99, 107, byte_format="q")[0],
            "low_price_of_the_day": self._unpack_data(binary_data, 107, 115, byte_format="q")[0],
            "closed_price": self._unpack_data(binary_data, 115, 123, byte_format="q")[0]
        }        
        try:
