...
#         if self.to_buy and not self.to_sell and self.waiting_for_sell \
...
#                 write_logs("SOLD", token, self.price, "Profit", f"LTP > 1.10* buying price -> {ltp_price} > {self.price}")
#                 self.price = ltp_price
#                 self.trade_details['datetime'] = datetime.now()
...
#                 write_logs("SOLD", token, self.price, "Profit", f"Latest made candle High > 1.10*buying price -> {data[selling_OHLC1].iloc[1]} > {selling_OHLC1_multiplier} *{self.price}")
...
#                 write_logs("SOLD", token, self.price, "Loss", f"Latest made candle Low < 0.95*buying price -> {data[selling_OHLC2].iloc[1]} <= {selling_OHLC2_multiplier} {self.price}")
