...
smart = SmartConnect(api_key=os.getenv("API_KEY"))
ltp_smart = SmartConnect(api_key=os.getenv("API_KEY"))

# Initialize SmartApiDataProvider instance
data_provider = SmartApiDataProvider(smart, ltp_smart)
...
# Endpoints for SmartApiDataProvider methods
@app.post("/fetch-candle-data/", tags=["SmartApi"])
async def fetch_candle_data(request: CandleRequest):
    """
    Endpoint to fetch candle data.
    """
    try:
        data = data_provider.fetch_candle_data(request.token, request.interval)
        return {"success": True, "data": data}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error fetching candle data: {str(e)}")


@app.post("/fetch-ltp-data/", tags=["SmartApi"])
async def fetch_ltp_data(request: Token):
    """
    Endpoint to fetch LTP data.
    """
    try:
        data = data_provider.fetch_ltp_data(request)
        return {"success": True, "data": data}
    except Exception as e:
        raise HTTPException(status_code=500, detail=f"Error fetching LTP data: {str(e)}")


# Middleware and startup/shutdown configurations
origins = [
    "localhost:3000",
    "http://127.0.0.1:3000",
    "http://localhost:3000",
